    ...
    for line in fin:  # each line is a json data that stores a CCIG/CIG graph
        ...
        for i, row in enumerate(v_texts):
            ...
            for j, val in enumerate(row):
                ...
                sent_idx = right_pad_zeros_1d([word_to_ix[w.decode("utf-8")] for w in val if w in word_to_ix], max_len)
        ...
        adj_3 = adj + adj.T
        '''
        adj_tfidf = adj_tfidf + adj_tfidf.T.multiply(adj_tfidf.T > adj_tfidf) - adj_tfidf.multiply(adj_tfidf.T > adj_tfidf)
        # 进行归一化，对应于论文中的A^=(D~)^0.5 A~ (D~)^0.5, 本代码实现的是A^=(D~)^-1 A~
        # A^=I+A
        adj_tfidf = normalize(adj_tfidf + sp.eye(adj_tfidf.shape[0]))
