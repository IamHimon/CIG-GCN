...
from src.models.CCIG.data.girvan_newman import *
from src.models.CCIG.data.sentence_pair_score import *
from src.models.CCIG.data.resource_loader import *
...
def get_concept_communities(sentences, concepts, betweenness_threshold_coef, max_c_size, min_c_size):
    ...
    concept_communities = []    # 所有concept的信息
    for c in distinct_c:
...
def construct_ccig(sentences, concepts, title=None, use_cd=True, betweenness_threshold_coef=1.0, max_c_size=10,
    ...
    # initialize vertex properties
    vprop_name = g.new_vertex_property("string")    # 节点名字，如果是concept，用空格join到一起作为名字
    vprop_concepts = g.new_vertex_property("vector<string>")    # 放concept内容节点
    vprop_sentidxs = g.new_vertex_property("vector<int32_t>")   # 放sentence指向此concept结点的sentence id
    ...
    i = 1
    for community in concept_communities:  # 每个concept
    ...
    # create edges
    eprop_name = g.new_edge_property("string")              # 边的名字，concepts中的全部关键词拼接起来
    eprop_concepts = g.new_edge_property("vector<string>")  # 放边连接的两个节点的concepts中的全部关键词
    eprop_sentidxs = g.new_edge_property("vector<int32_t>") # 放这个节点出发的所有sentence_idx
    eprop_weight_numsent = g.new_edge_property("double")    # 边权重，sentence数量
    eprop_weight_tfidf = g.new_edge_property("double")      # 边权重，tfidf

    # edges by connective sentences
    # 根据sentences构造边
    for sent_idx in range(len(sentences)):  # 每个句子
        sent = sentences[sent_idx]
        words = str(sent).split()
        intersect = set(words).intersection(set(concepts))  # 取当前sentence中的关键词
        if len(intersect) == 0:
            continue
        related_vertexidxs = []
        for c in intersect:
    ...
    for i in range(num_v):
        for j in range(i, num_v):   # O(n^2)
            ...
            text2 = " ".join(text2)         # 把所有相关的sentence拼接起来，
    ...
    g.vertex_properties["katz"] = katz(g, weight=eprop_weight_tfidf)    # the Katz centrality of a node is a measure of centrality in a network
    ...
    # save properties as internal properties
    # 任何创建的property可以设定为某个graph的自身属性，这样就意味着，当你copy和存储图是，这些属性会更图一起存在。
    # 把这些属性插入到图中，同时取一个名字
    g.vertex_properties["name"] = vprop_name
...
if __name__ == "__main__":
    ...
    print(sentences)
    # g2 = construct_ccig(sentences, concepts, use_cd=False)
    # draw_ccig(g2, "cig.pdf")
