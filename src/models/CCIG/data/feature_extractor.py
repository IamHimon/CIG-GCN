# coding=utf-8
from src.models.CCIG.data.config import *
import json
import pandas as pd
import numpy as np
import multiprocessing as mp
from functools import partial
from src.models.CCIG.data.ccig import *
from src.models.CCIG.data.resource_loader import load_W2V_VOCAB
from src.models.CCIG.data.sentence_score import *
from src.models.CCIG.data.sentence_pair_score import *
from src.models.CCIG.data.bm25 import *
...
def assign_graph_node_features(g, sentences1, sentences2, title1=None, title2=None):
    ...
    2.是否是空节点
    3.否有左右节点
    4.Term_based similarities (5维)
    命名为"features"

    vprop_features_w2v：
    所有title的token作为特征，命名为"texts"
    ...
    """
    vprop_features_manual = g.new_vertex_property("vector<double>")     #节点feature value
    vprop_features_w2v = g.new_vertex_property("vector<string>")        # 所有词
    ...
    for v in g.vertices():
        ...
        # string similarities
        v_features.extend(calc_text_pair_features(text1, text2))    # 添加feature
...
def assign_graph_edge_weights(g, sentences1, sentences2):
    ...
    eprop_weight_position = g.new_edge_property("double")   # 句子的位置
    eprop_weight_textrank = g.new_edge_property("double")    # 计算句子的textrank
...
def assign_graph_features(g, category, time1, time2,
    ...
    news_features = []      # 时间相差也作为一个特征，time gap
    ...
    sents1_01 = " ".join(sentences1[0:1])   # 开头一句
    sents2_01 = " ".join(sentences2[0:1])
    sents1_02 = " ".join(sentences1[0:2])   # 开头两句
    sents2_02 = " ".join(sentences2[0:2])
    sents1_03 = " ".join(sentences1[0:3])   # 开头两句
    sents2_03 = " ".join(sentences2[0:3])
    sents1_04 = " ".join(sentences1[0:4])   # 开头两句
    sents2_04 = " ".join(sentences2[0:4])
    sents1_05 = " ".join(sentences1[0:5])   # 开头两句
    sents2_05 = " ".join(sentences2[0:5])
    content_features = calc_text_pair_features(content1, content2)  # 计算五个特征值
...
def save_graph_features_to_file(gs, outfile, draw_fig=False):
    ...
    for g in gs:
        ...
        v_features_mat = []     # 节点feature 矩阵
        v_texts_mat = []        # 节点的text 矩阵
        ...
        for idx in range(num_v):
            ...
            v_texts_mat.append(list(g.vertex_properties["texts"][g.vertex(idx)]))  # NOTICE: vector<string> to list,
...
def dataset2featurefile(infile, outfile,
    ...
    """
    """
    gs = extract_align_graphs_from_docpair_data(    # 构造得到所有cig
        infile,
        col_label, col_category,
        col_time1, col_time2,
        col_text1, col_text2,
        ...
        betweenness_threshold_coef, max_c_size, min_c_size)
    save_graph_features_to_file(gs, outfile, draw_fig)  # 存储结果
...
if __name__ == "__main__":
    ...
    dataset2featurefile(
        ...
        draw_fig=True, parallel=False, extract_range=range(10), print_fig=True)

    # process data
    # dataset2featurefile(
    #     "../../../../data/raw/event-story-cluster/same_event_doc_pair.txt",
    #     "../../../../data/processed/event-story-cluster/same_event_doc_pair.cd.cp.json",
    #     "label", "category1", "time1", "time2", "content1", "content2",
    #     ["keywords1", "ner_keywords1"], ["keywords2", "ner_keywords2"],
