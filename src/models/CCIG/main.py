...
from src.models.CCIG.loader import *
from src.models.CCIG.models import *
from src.models.CCIG.models.ema import EMA
from src.models.CCIG.util.nlp_utils import *
from src.models.CCIG.util.exp_utils import set_device, set_random_seed, summarize_model
...
parser.add_argument('--num_data', type=int, default=100, help='maximum number of data samples to use.')
...
parser.add_argument('--use_gcn', action='store_true', default=True, help='use GCN in model.')
parser.add_argument('--num_gcn_layers', type=int, default=2, help='number of gcn layers')
parser.add_argument('--use_cd', action='store_true', default=True, help='use community detection in graph.')
parser.add_argument('--use_siamese', action='store_true', default=False, help='use siamese encoding for vertices.')
parser.add_argument('--use_vfeatures', action='store_true', default=True, help='use vertex features.')
...
path = "/Users/himon/Jobs/class/paper9/part3/ArticlePairMatching/data/processed/" + args.inputdata
...
adjs = adjacent_dict[args.adjacent]         # tfidf
g_vertices = g_vertices_dict[args.vertice]  # 节点中心度指标
...
def train_epoch(epoch, step):
    ...
    for i in idx_train:
        ...
        v_feature = v_features_list[i]  # 节点的特征向量
        g_feature = g_features[i]   # 图的特征向量
        adj = adjs[i]   # A^=(D~)^-1 A~
        # g_vertice = g_vertices[i]   # 节点中心度指标
        label = labels[i]  # must add []

        # calculate loss and back propagation
        model.train()
        optimizer.zero_grad()
        output = model(w2v_idxs_l, w2v_idxs_r, v_feature, adj, g_feature)  # 一个batch就是一个CIG，一个smaple
        ...
        # 使用EMA（指数移动平均）这个方法对模型的参数做平均，以求提高测试指标并增加模型鲁棒
        if args.use_ema:
            ema(model, step)
        step += 1
    ...
    # print(" loss_train:%s, acc_train:%s, f1_train:%s" % (loss_train, acc_train, f1_train))
    # Evaluate validation set performance separately,
    loss_val, acc_val, f1_val, outputs_val = test(
        model, v_texts_w2v_idxs_l_list, v_texts_w2v_idxs_r_list, v_features_list, adjs, g_features, g_vertices, labels,
        idx_val)
...
def train(args, fout):
    ...
    for epoch in range(args.epochs):
        ...
        test_loss, test_acc, test_f1, outputs_test = test(
            model, v_texts_w2v_idxs_l_list, v_texts_w2v_idxs_r_list, v_features_list, adjs, g_features, g_vertices,
            labels, idx_test)
        print(
            "Test set results:",
...
def test(model, v_texts_w2v_idxs_l_list, v_texts_w2v_idxs_r_list, v_features_list, adjs, g_features, g_vertices, labels,
         idxs):
    model.eval()
    outputs = []
    output_p = []
    ...
    for i in idxs:
        ...
        label = labels[i]
        output = model(w2v_idxs_l, w2v_idxs_r, v_feature, adj, g_feature)
